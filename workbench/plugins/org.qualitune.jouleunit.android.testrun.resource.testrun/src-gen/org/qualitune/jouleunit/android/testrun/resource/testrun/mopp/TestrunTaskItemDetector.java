...
/**
 * The TestrunTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * TestrunTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class TestrunTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.qualitune.jouleunit.android.testrun.resource.testrun.mopp.TestrunTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.qualitune.jouleunit.android.testrun.resource.testrun.mopp.TestrunTaskItem> foundItems = new java.util.ArrayList<org.qualitune.jouleunit.android.testrun.resource.testrun.mopp.TestrunTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.qualitune.jouleunit.android.testrun.resource.testrun.mopp.TestrunTaskItem(keyword, message, localLine, offset, end));
