...
public class TestrunDefaultNameProvider implements org.qualitune.jouleunit.android.testrun.resource.testrun.ITestrunNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.qualitune.jouleunit.android.testrun.resource.testrun.util.TestrunEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
